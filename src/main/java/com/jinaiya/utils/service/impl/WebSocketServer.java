...
import com.jinaiya.utils.handler.WebSocketServerHandler;
import com.jinaiya.utils.service.MyHttpService;
import com.jinaiya.utils.service.MyWebSocketService;
...
public class WebSocketServer implements MyHttpService, MyWebSocketService {
    ...
    public void start(){
        ...
        try {
            ...
            channelFuture.channel().closeFuture().addListener((ChannelFutureListener) channelFuture12 ->
                    logger.info("server channel {} closed.", channelFuture12.channel())).sync();
    ...
    public void handleFrame(ChannelHandlerContext ctx, WebSocketFrame frame) {
        ...
        if (frame instanceof TextWebSocketFrame) {
            ...
            logger.info("receive textWebSocketFrame from channel: {} , 目前一共有{}个在线", ctx.channel(), channelMap.size());
            //发给其它的 channel  (群聊功能)
            for (Channel ch: channelMap.values()){
                ...
                logger.info("消息已发送给{}", ch);
                logger.info("write text: {} to channel: {}", textWebSocketFrame, ctx.channel());
            }
            return;
        }
        /**
        ...
        if (frame instanceof CloseWebSocketFrame){
            logger.info("receive closeWebSocketFrame from channel: {}", ctx.channel());
            //获取到握手信息
